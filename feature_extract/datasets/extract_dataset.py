...
class VideoSet(torch.utils.data.Dataset):
    ...
    def _get_frames(self):
        ...
        frames = np.empty(shape=(total_count+1,int(self.sample_height),int(self.sample_width),3))
        print("total frame: ",total_count)
        count = 0
        while(True):
            if count%1000==0:
            ...
            #print(f'original_shap: {height, width}')
            cropped_frame = frame[int(height*0.1):int(height*0.9), int(width*0.45):int(width*0.9)]
            #print(f'cropped_image_shap: {cropped_frame.shape[:2]}')
            frame = cv2.resize(cropped_frame,(self.sample_height,self.sample_width),interpolation = cv2.INTER_LINEAR)
            #cv2.imwrite("/home/lxj/temp_folder/"+"test"+str(count)+".jpg", frame)
            pos = int(count//12)
            frames[pos,:,:,:] = frame
