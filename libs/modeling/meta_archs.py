...
class TriDet(nn.Module):
    ...
    def forward(self, video_list):
        ...
        save_name = video_id+"_logits.pkl"
        dir_path = "/home/lxj/project/surgplan/LUO/test_only/TriDet/slowfast_pred"
        save_path = os.path.join(dir_path,save_name)
        torch.save(out_cls_logits[0][0].to("cpu"),save_path)
    ...
    def inference_single_video(
        ...
        for cls_i, offsets_i, pts_i, mask_i, sb_cls_i, eb_cls_i in zip(
            ...
            reference_prob = cls_i.sigmoid() * mask_i.unsqueeze(-1)
            ...
            #smoothing method
            """ref_score_list = torch.zeros(pred_segs[keep_idxs2].shape[0])
            ref_index_list = torch.zeros(pred_segs[keep_idxs2].shape[0])
            for i,(s,e) in enumerate(pred_segs[keep_idxs2]):
                s = s/scale
                s = s.round().int()
                s = max(s,0)
                e = e/scale
                e = e.round().int()
                ref_scores = reference_prob[s:e+1].mean(0)
                ref_score = ref_scores.max()
                ref_score_list[i] =   ref_score
                ref_index = ref_scores.argmax()
                ref_index_list[i] =ref_index
            """
            #print(sum(ref_index_list == cls_idxs[keep_idxs2].to("cpu")))
            scale = scale*2
            # *_all : N (filtered # of segments) x 2 / 1
            """segs_all.append(pred_segs[keep_idxs2])
            scores_all.append(ref_score_list)
            cls_idxs_all.append(ref_index_list)"""
